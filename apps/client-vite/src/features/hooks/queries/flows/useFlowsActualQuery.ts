...
const useFlowsActualQuery = (
  ...
  const endDateTotalMoney = {} as { [asset: string]: number };
  ...
  const {
    ...
    ...queryFactory.actualInfinite(endDate, size, assets, transactionType),
    placeholderData: {
      pageParams: [],
      pages: [
        {
  ...
  useEffect(() => {
    ...
    const makeValues = (prevAccData, dateIndexMapping) => {
      ...
      const dataInit: FlowDataValuesType = newlyFetchedPageInfo.data.reduce(
        (acc, cur) => ({
          ...
          [cur.asset]: new Array(
            date.length +
              (prevAccData.values[cur.asset] ? 0 : prevAccData.date.length) // 이전 내용에 존재하면 현재 fetch 내용만 고려. 처음 들어온거면 이전 고려해서 데이터 쌓아야 하니.
      ...
      const prevAssetInit: FlowDataValuesType = Object.entries(
        ...
        return {
          ...acc,
          [asset]: new Array(date.length).fill(makeValueInfoInit()),
        };
      }, {});
      ...
      for (let i = 0; i < newlyFetchedPageInfo.data.length; i++) {
        ...
        //mutation
        data[newlyFetchedPageInfo.data[i].asset][dataIdx].metaData.push(
          valueInfo
        );
      }
      ...
      Object.entries(data).forEach(([asset, dataOnAsset]) => {
        dataOnAsset.forEach((valueInfo, idx) => {
          if (valueInfo.metaData.length > 0) {
            ...
            const localCurrencyValue =
              lastMetaData.accShares * lastMetaData.price;
            const localExchangeRate =
              metaData[metaData.length - 1].exchangeRate;
            ...
            const prevValueInfo =
              idx > 0
                ...
                : { krwValue: 0, localCurrencyValue: 0 }; //dataOnAsset prevAccData[asset][0].value;
    ...
    setAccData((prevAccData) => {
      const newValues = makeValues(prevAccData, dateIndexMapping);
      //test
      ...
      if (date[date.length - 1] === prevAccData.date[0]) {
        ...
        const metaDataOnDuplicatedDate = Object.entries(newValues).reduce(
          (acc, [asset, valueInfos]) => {
            //test
            console.log('prevAccData.values,asset', prevAccData.values, asset);

            const prevMetaData = prevAccData.values[asset]?.[0].metaData ?? []; // 최근 날짜 대한. prev에 해당 asset은 없을 수 있으니.
            const newMetaData = valueInfos[valueInfos.length - 1].metaData; // 옛날 날짜 대한.
            return { ...acc, [asset]: [...newMetaData, ...prevMetaData] };
          },
          {} as { [asset: string]: FlowDataValuesMetaDataType[] }
        );
          ...
          (acc, [asset, valueInfos]) => {
            ...
            return {
              ...
              [asset]: [
                {
                  ...recentValues,
                  metaData: metaDataOnDuplicatedDate[asset],
                },
              ],
            };
          },
          ...
          (acc, [asset, newValueInfos]) => {
            ...
            return {
              ...
              [asset]: [
                ...
                ...(dataOnDuplicatedDate[asset] ?? []), // asset에서 중복된게 없을수도 있으니.
        ...
        //   (acc, [asset, prevValueInfos]) => {
        //     const prevCorrectDataOnAsset = prevValueInfos.slice(
        //       1,
        //       prevValueInfos.length
        //     );
        //     const newCorrectDataOnAsset = newValues[asset].slice(
        //       0,
        //       newValues[asset].length - 1
        //     );
        //     return {
        //       ...acc,
        //       [asset]: [
        //         ...prevCorrectDataOnAsset,
        //         dataOnDuplicatedDate[asset],
        //         ...newCorrectDataOnAsset,
        //       ],
        //     };
        //   },
      ...
      const resValues = Object.entries(newValues).reduce(
        (acc, [asset, valueInfos]) => {
          return {
            ...acc,
            [asset]: [...valueInfos, ...(prevAccData.values?.[asset] ?? [])], // 없을 수도 있으니.
          };
        },
        {}
      );
